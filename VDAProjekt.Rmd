...
```{r plot(SalesPerGamePerPersonperYear), include=TRUE, echo = FALSE, message=FALSE, results='markup', }
grouped <- vgsales  %>%                                 
  group_by(Year) %>%
  dplyr::summarize(gr_sum = sum(Global_Sales), Population = sum(WorldPopulation$Year)) %>% filter(Year!='N/A')%>%filter(Year!=2020) %>% filter(Year!='2017') %>%
  as.data.frame()
